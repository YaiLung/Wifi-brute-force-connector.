...
def wifi_password(wifi_name):
    ...
    with open(wifi_dic_path, 'r') as f:
        ...
        for pwd in f:
            ...
            # create a thread for this attempt
            thread = threading.Thread(target=check_password, args=(interface, pwd))
            thread.start()


def check_password(interface, pwd):
...
def main():
    ...
    while not exit_flag:
        try:
            ...
            wifi_password(wifi_list[target_num][1])
            print('-' * 38)
            exit_flag = 1
